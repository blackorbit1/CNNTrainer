...
while True:
    ...
    ║                 ENTRAINEUR DE CNN                    ║
    ║                                                      ║
    ║ - tappez 'c' pour continuer l'entrainement d'un CNN  ║
    ║ - tappez 'n' pour entrainer un nouveau CNN           ║
    ║ - tappez 'f' pour fermer                             ║
    ║                                                      ║
    ╚═══════════════════════════════╝
    """)
...
def setup_to_transfer_learn(model, base_model, pas_preentrainement):
    """Freeze all layers and compile the model"""
    for layer in base_model.layers:
        layer.trainable = False
    rmsprop = RMSprop(lr=pas_preentrainement, rho=0.9, epsilon=None, decay=0.0)
...
def setup_to_finetune(model, pas_entrainement):
...
if preentrainement:
    # transfer learning
    setup_to_transfer_learn(model, base_model, pas_preentrainement)
...
# fine-tuning
setup_to_finetune(model, pas_entrainement)
